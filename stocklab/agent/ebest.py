...
class EBest:
    ...
    def __init__(self, mode=None):
        ...
        if mode not in ["PROD", "DEMO", "ACE"]:
            raise Exception("Need to run_moe(PROD or DEMO)")
    ...
    def _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields):
        ...
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN:
            ...
            print("waiting for execute query... current query cnt:",
                  len(self.query_cnt))
            self.query_cnt = list(filter(lambda x: (
                datetime.today() - x).total_seconds() < EBest.LIMIT_SECONDS, self.query_cnt))
    ...
    def order_stock(self, code, qty, price, bns_type, order_type):
        ...
        out_params = ["OrdNo", "OrdTime", "OrdMktCode", "OrdPtnCode",
                      "ShtnIsuNO", "MgempNo", "OrdAmt", "SpotOrdQty", "IsuNm"]
