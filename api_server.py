...
class CodeList(Resource):
    ...
    def get(self):
        ...
        if market == "0":
            ...
            results = list(mongodb.find_items(
                {"시장구분": market}, "stocklab", "code_info"))
        result_list = []
        for item in results:
            code_info = {}
...
class Price(Resource):
    ...
    def get(self, code):
        ...
        start_date = request.args.get(
            'start_date', default=default_start_date.strftime("%Y%m%d"), type=str)
...
class OrderList(Resource):
    def get(self):
        ...
        if status == 'all':
            result_list = list(mongodb.find_items(
                {}, "stocklab_demo", "order"))
        elif status in ["buy_ordered", "buy_completed", "sell_ordered", "sell_completed"]:
            result_list = list(mongodb.find_items(
                {"status": status}, "stocklab_demo", "order"))
        else:
            return {}, 404
